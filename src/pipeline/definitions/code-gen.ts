      ...
      buildPrompt: (ctx) => {
        ...
        return {
          system: [
            ...
            '',
            'ENGINEERING PRINCIPLES (the spec MUST embody these — apply PROPORTIONALLY to project size;',
            'a small script should NOT grow speculative architecture):',
            '- Single responsibility: each module/function does one thing.',
            '- Explicit contracts at every boundary: validate all inputs and outputs; trust nothing incoming.',
            '- Centralized error handling: one consistent error path/shape, NOT scattered ad-hoc try/catch.',
            '  Define canonical error categories; translate to transport (HTTP/CLI exit/etc.) only at the edge.',
            '- Least privilege & explicit side effects: components touch only what they must.',
            '- YAGNI / KISS: build ONLY what the request needs; explicit data flow, minimal hidden branching.',
            '- Replaceable via clear contracts: implementations swappable without system-wide refactors.',
            '- Predictable, auditable failures: log errors/state changes; retries idempotent.',
            'Scale the rigor to the task: a 50-line CLI needs clean error handling and input validation, NOT a',
            'domain-error taxonomy with transport-translation layers. A service or anything with real boundaries does.',
            '',
