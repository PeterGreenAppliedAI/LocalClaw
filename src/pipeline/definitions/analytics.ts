    ...
    // Stage 1: Load data + compute complete report in Python (deterministic)
    {
      ...
      execute: async (ctx) => {
        ...
        const pyCode = [
          ...
          '    label_keywords = ["description", "name", "item", "vendor", "product"]',
          '    label_col = None',
          '    for col in categorical_cols:',
    ...
    // Stage 2: Generate charts from the data (deterministic Python)
    {
      ...
      execute: async (ctx) => {
        ...
        const pyCode = [
          ...
          '    main_num = numeric_cols[-1] if numeric_cols else None',
          '',
          'group_cols = [c for c in categorical_cols if 2 <= df[c].nunique() <= 15]',
          'if not group_cols:',
          '    group_cols = [c for c in categorical_cols if df[c].nunique() <= 30][:2]',
          '',
          '# Clean NaN from all columns before charting — matplotlib cannot handle NaN in categorical data',
          'df_clean = df.dropna()',
          '',
          '# Chart 1: Category breakdown bar chart',
          'if main_num and group_cols:',
          '    cat_col = group_cols[0]',
          '    gb = df_clean.groupby(cat_col)[main_num].sum().sort_values(ascending=True)',
          ...
          '        label_col = categorical_cols[0]',
          '    if label_col:',
          '        top = df_clean.nlargest(10, main_num)',
