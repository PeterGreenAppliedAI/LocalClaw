...
import { handleListBuilds, handleGetBuild, handleDeleteBuild, handleCodeBuild } from './handlers/code.js';
...
export async function handleConsoleRequest(
  ...
  try {
    ...
    if (path === 'code/build' && method === 'POST') {
      await handleCodeBuild(req, res, deps);
      return true;
    }
