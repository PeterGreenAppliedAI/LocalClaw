...
**Status:** Live and enabled. Remaining gap (Phase 2): cited-source-only still can't disprove a wrong fact from a faithfully-cited source — e.g. this run kept "Groq licensed by NVIDIA … December 2024" (date wrong, should be 2025) because the source supported it. Catching that needs the Tier-1 official-source cross-check. Trust the human side-by-side read over the judge score ([[feedback_regressions]]).
